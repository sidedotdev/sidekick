...
func main() {
	...
	// Migrate task changes for each workspace
...
func migrateTaskChanges(ctx context.Context, redisStreamer *redis.Streamer, js *jetstreamClient.Streamer, workspaceId string) error {
	...
	lastMessageId, err := getLastMigratedMessageId(workspaceId)
	if err != nil {
	...
	for {
		...
		err = saveLastMigratedMessageId(workspaceId, nextMessageId)
		if err != nil {
...
func getLastMigratedMessageId(workspaceId string) (string, error) {
	// TODO: Implement this function to retrieve the last migrated message ID from a persistent store
	return "", nil
}

func saveLastMigratedMessageId(workspaceId, messageId string) error {
