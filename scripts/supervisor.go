...
var defaultProcesses = []ProcessConfig{
	{
		Name:            "temporal",
		Command:         "sh",
		Args:            []string{"-c", "go build -buildvcs=true -o side-temporal ./cmd/temporal && SIDE_LOG_LEVEL=0 SIDE_APP_ENV=development ./side-temporal"},
		PrebuildCommand: "go",
		PrebuildArgs:    []string{"build", "-buildvcs=true", "-o", "side-temporal", "./cmd/temporal"},
		WatchGlobs:      []string{"cmd/temporal/**/*.go", "temporal/**/*.go"},
	},
	{
		Name:            "api",
		Command:         "sh",
		Args:            []string{"-c", "go build -buildvcs=true -o side-api ./api/main && SIDE_LOG_LEVEL=0 SIDE_APP_ENV=development ./side-api"},
		PrebuildCommand: "go",
		PrebuildArgs:    []string{"build", "-buildvcs=true", "-o", "side-api", "./api/main"},
		WatchGlobs:      []string{"**/*.go"},
	},
	{
		Name:            "worker",
		Command:         "sh",
		Args:            []string{"-c", "go build -buildvcs=true -o side-worker ./worker/main && SIDE_LOG_LEVEL=0 SIDE_APP_ENV=development ./side-worker"},
		PrebuildCommand: "go",
		PrebuildArgs:    []string{"build", "-buildvcs=true", "-o", "side-worker", "./worker/main"},
		WatchGlobs:      []string{"**/*.go"},
	},
	{
		Name:       "frontend",
