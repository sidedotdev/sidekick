<template>
  ...
  <div v-if="showIdeSelector" class="ide-selector-overlay" @click.self="cancelIdeSelection">
    <div class="ide-selector-dialog">
      <h3>Open file in...</h3>
      <div class="ide-selector-buttons">
        <button @click="selectIde('vscode')" class="ide-button">
          <VSCodeIcon />
          <span>VS Code</span>
        </button>
        <button @click="selectIde('intellij')" class="ide-button">
          <IntellijIcon />
          <span>IntelliJ</span>
        </button>
      </div>
      <button @click="cancelIdeSelection" class="ide-cancel-button">Cancel</button>
    </div>
  </div>
</template>
...
const fetchFlowSubflows = async (flowId: string) => {
  try {
    const response = await fetch(`/api/v1/workspaces/${store.workspaceId}/flows/${flowId}/subflows`);
    if (response.ok) {
    ...
    console.error(`Error fetching subflows for flow ${flowId}:`, err);
  }
};
