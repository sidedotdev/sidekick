...
Due to how edit blocks work, the prompt for resolving conflicts must mention using sed to replace conflict markers with CONFLICT_START/DIVIDER/END, so that the conflict markers aren't confused with the edit block format delimiters.