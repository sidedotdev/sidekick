...
func (s Storage) PersistWorktree(ctx context.Context, worktree domain.Worktree) error {
	...
	worktreeKey := fmt.Sprintf("worktree:%s:%s", worktree.WorkspaceId, worktree.Id)
	...
	workspaceWorktreesKey := fmt.Sprintf("workspace:%s:worktrees", worktree.WorkspaceId)
...
func (s Storage) GetWorktrees(ctx context.Context, workspaceId string) ([]domain.Worktree, error) {
	workspaceWorktreesKey := fmt.Sprintf("workspace:%s:worktrees", workspaceId)
