...
func (s Storage) PersistWorktree(ctx context.Context, worktree domain.Worktree) error {
	...
	flowWorktreesKey := fmt.Sprintf("%s:%s:worktrees", worktree.WorkspaceId, worktree.FlowId)
...
func (s Storage) GetWorktreesForFlow(ctx context.Context, workspaceId, flowId string) ([]domain.Worktree, error) {
	flowWorktreesKey := fmt.Sprintf("%s:%s:worktrees", workspaceId, flowId)
...
func (s Storage) DeleteWorktree(ctx context.Context, workspaceId, worktreeId string) error {
	...
	if worktree.FlowId != "" {
		flowWorktreesKey := fmt.Sprintf("%s:%s:worktrees", workspaceId, worktree.FlowId)
		err := pipe.SRem(ctx, flowWorktreesKey, worktreeId).Err()
		if err != nil {
			return fmt.Errorf("failed to remove worktree from flow set: %w", err)
		}
	}
