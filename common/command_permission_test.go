...
func TestMatchPattern(t *testing.T) {
	tests := []struct {
		...
		// Word boundary: prefix match must not match longer command names
		{
			name:          "word boundary - exact command with args",
			pattern:       "ls",
			command:       "ls -la",
			expectMatch:   true,
			expectMatches: []string{"ls"},
		...
		{
			name:        "word boundary - rejects longer command",
			pattern:     "ls",
			command:     "lsof /tmp",
			expectMatch: false,
		},
		{
			name:          "word boundary - exact match no args",
			pattern:       "cat",
			command:       "cat",
			expectMatch:   true,
			expectMatches: []string{"cat"},
		...
		{
			name:        "word boundary - rejects extended word",
			pattern:     "cat",
			command:     "caterpillar",
			expectMatch: false,
		},
		{
			name:          "word boundary - subcommand with args",
			pattern:       "git push",
			command:       "git push origin main",
			expectMatch:   true,
			expectMatches: []string{"git push"},
		...
		{
			name:        "word boundary - rejects extended subcommand",
			pattern:     "git push",
			command:     "git pushover origin",
			expectMatch: false,
		},
		{
			name:          "word boundary - bunx tsc with flags",
			pattern:       "bunx tsc",
			command:       "bunx tsc --noEmit",
			expectMatch:   true,
			expectMatches: []string{"bunx tsc"},
		},
		{
			name:        "word boundary - bunx rejects similar command",
			pattern:     "bunx tsc",
			command:     "bunx tscfake",
			expectMatch: false,
		},
		{
			name:        "word boundary - bunx rejects hyphenated command",
			pattern:     "bunx tsc",
			command:     "bunx tsc-evil",
			expectMatch: false,
		},
		{
			name:          "word boundary - npm run with space separator",
			pattern:       "npm run",
			command:       "npm run build",
			expectMatch:   true,
			expectMatches: []string{"npm run"},
		},
		{
			name:        "word boundary - npm rejects extended command",
			pattern:     "npm run",
			command:     "npm runscript",
			expectMatch: false,
		},
		{
			name:          "word boundary - pattern ending in non-word char",
			pattern:       "echo ",
			command:       "echo hello",
			expectMatch:   true,
			expectMatches: []string{"echo "},
		},
		{
			name:          "word boundary - pattern with slash",
			pattern:       "cd /",
			command:       "cd /tmp",
			expectMatch:   true,
			expectMatches: []string{"cd /"},
		},
		{
			name:          "word boundary - multi-level subcommand",
			pattern:       "docker compose up",
			command:       "docker compose up -d",
			expectMatch:   true,
			expectMatches: []string{"docker compose up"},
		},
		{
			name:        "word boundary - multi-level rejects extension",
			pattern:     "docker compose up",
			command:     "docker compose update",
			expectMatch: false,
		},
	}
