	...
	for {
		...
		*chatHistory = append(*chatHistory, chatResponse.ChatMessage)
		...
		if len(chatResponse.ToolCalls) > 0 {
			...
			for _, tc := range chatResponse.ToolCalls {
				...
				doneTool.Name: func(dCtx DevContext, toolCall llm.ToolCall) (ToolCallResponseInfo, error) {
					if doneCalledAlongsideOtherWork {
						return ToolCallResponseInfo{
							...
							Response:     "The `done` tool must be called on its own, without any other tool calls or edit blocks in the same response. If you still have more work to do, continue working. Once all work is complete, call `done` by itself.",
						}, nil
					}
...
func buildAuthorEditBlockInput(dCtx DevContext, codingModelConfig common.ModelConfig, chatHistory *[]llm.ChatMessage, promptInfo PromptInfo, doneRequired bool, hasPlan bool) llm.ToolChatOptions {
	...
	case InitialCodeInfo:
		v := workflow.GetVersion(dCtx, "apply-edit-blocks-immediately", workflow.DefaultVersion, 1)
		applyImmediately := v >= 1 && !dCtx.RepoConfig.DisableHumanInTheLoop
		content = renderAuthorEditBlockInitialPrompt(dCtx, info.CodeContext, info.Requirements, applyImmediately, doneRequired)
...
func renderAuthorEditBlockInitialPrompt(dCtx DevContext, codeContext, requirements string, applyEditBlocksImmediately, doneRequired bool) string {
...
func renderAuthorEditBlockInitialDevStepPrompt(dCtx DevContext, codeContext, requirements, planContext, currentStep string, applyEditBlocksImmediately, doneRequired bool) string {
	data := map[string]interface{}{
		"codeContext":                     codeContext,
		"requirements":                    requirements,
