...
func addToolCallResponse(chatHistory *llm2.ChatHistoryContainer, info ToolCallResponseInfo) {
	if _, ok := chatHistory.History.(*llm2.Llm2ChatHistory); ok {
	...
	for _, b := range info.ToolResultContent {
		if b.Type != llm2.ContentBlockTypeText {
			panic("non-text tool results require llm2 chat history format")
		}
	}
	chatHistory.Append(llm.ChatMessage{
